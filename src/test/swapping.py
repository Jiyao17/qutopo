...
def swapping(vsrc:VertexSource, params, repeat=1):
    """
    ...
    for i, density in enumerate(densities):
        ...
        for j in range(repeat):
            ...
            prob = params['swap_prob']
            pickle.dump(
                (densities, ys1, ys2, y1_labels, y2_labels),
                open(f'./result/swapping/efficiency-{vset.name}-{prob}-avg.pkl', 'wb')
            )
            ...
            plot_lines(
                ...
                filename=f'./result/swapping/efficiency-objective-{vset.name}-{prob}.png'
            )
            ...
            plot_lines(
                ...
                filename=f'./result/swapping/efficiency-time-{vset.name}-{prob}.png'
            )
                
def load_plot_result(vset:VertexSet, prob):
    densities, ys1, ys2, y1_labels, y2_labels = pickle.load(
        open(f'./result/swapping/efficiency-{vset.name}-{prob}-avg.pkl', 'rb')
        )
...
if __name__ == '__main__':
    ...
    # load_plot_result(VertexSource.RENATOR, 0.5)

    probs = [0.5]
    for prob in probs:
        params = copy.deepcopy(HWParam)
        params['swap_prob'] = prob
        # swapping(VertexSource.EENET, params, repeat=1)
        # swapping(VertexSource.NOEL, params, repeat=1)
        # swapping(VertexSource.RENATOR, params, repeat=1)
        load_plot_result(VertexSource.RENATOR, prob)
