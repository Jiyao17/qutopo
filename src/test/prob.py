...
from ..utils.plot import plot_2y_lines, plot_lines, plot_simple_topology
...
def load_plot_result(vsrc:VertexSource):
    densities, ys1, ys2, y1_labels, y2_labels = pickle.load(
        open(f'./result/efficiency/efficiency-{vsrc.name}-avg.pkl', 'rb')
        )
    ...
    plot_lines(
        ...
        filename=f'./result/efficiency/efficiency-objective-{vsrc.name}-0.5.png'
    )
    ...
    plot_lines(
        ...
        yscale='log',
        adjust=(0.18, 0.16, 0.94, 0.94),
        filename=f'./result/efficiency/efficiency-time-{vsrc.name}-0.5.png'
    )
